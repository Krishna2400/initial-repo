...
#     if not intents_list:
#         # Log the unrecognized query
#         try:
#             with open('unrecognized_queries.txt', 'a') as f:
#                 f.write(user_input + '\n')
#         except Exception as e:
#             print(f"Error logging unrecognized query: {e}")
        
#         # Provide a helpful response with a link
#         return ("Sorry, I didn't quite catch that. "
#                 "Please check our help center for more information: "
#                 "https://www.yourwebsite.com/help")
...
def predict_class(sentence):
    bow = bag_of_words(sentence)
    res = model.predict(np.array([bow]))[0]
    ERROR_THRESHOLD = 0.6  # Further increase to avoid incorrect matches
    results = [[i, r] for i, r in enumerate(res) if r > ERROR_THRESHOLD]
    results.sort(key=lambda x: x[1], reverse=True)
    return_list = []
    for r in results:
