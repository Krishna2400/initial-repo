...
model = tf.keras.models.load_model('chatbot_transformer_model.keras')
...
def fuzzy_match_intent(query):
    """Fuzzy match for handling user typos or variations."""
    similar_intents = []
...
def handle_ambiguous_intent(query):
    """Handles ambiguous intents by suggesting similar ones."""
    similar_intents = fuzzy_match_intent(query)
    if similar_intents:
        return f"Did you mean: {', '.join(similar_intents)}?"
    else:
        return "Sorry, I couldn't understand your query. Could you clarify?"

# Chatbot Interaction with session context
def chat(session_id):
    print("GO! Bot is running!")
    
    while True:
        message = input(f"User ({session_id}): ")
        ...
        if get_context(session_id):
            ...
            if previous_context == 'canteen_menu':
                ...
                response = "Here is today's canteen menu!"
            else:
        ...
        if not predicted_intents:
            response = handle_ambiguous_intent(message)
        
        print(f"Bot: {response}")

# Example usage
chat(session_id="12345")
